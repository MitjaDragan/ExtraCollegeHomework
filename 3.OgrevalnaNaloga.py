...
while counter < len(kraji):
    distance = math.sqrt((kraji[counter][1])**2 + (kraji[counter][2]**2))
    print(kraji[counter][0], distance)
    counter = counter + 1

while counter2 < len(kraji):
    if (kraji[counter2][1]) >= 10 and (kraji[counter2][1]) <= 50:
        print(kraji[counter2][0])
    counter2 = counter2 + 1
