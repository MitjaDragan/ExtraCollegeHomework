...
east2 = "test"
south2 = "test"
while counter < (len(kraji)):
    if kraji[counter][2] >= north:
        north = kraji[counter][2]
        north2 = kraji[counter][0]
        b = counter
    if kraji[counter][1] >= east:
        east = kraji[counter][1]
        east2 = kraji[counter][0]
        a = counter
    if kraji[counter][1] <= west:
        west = kraji[counter][1]
        west2 = kraji[counter][0]
        c = counter
    if kraji[counter][2] <= south:
        south = kraji[counter][2]
        south2 = kraji[counter][0]
        d = counter
    counter = counter + 1

print(north2, "is most north")
print(south2, "is most south")
print(west2, "is most west")
print(east2, "is most east")

    
x = kraji[a][1] - (kraji[d][1])
y = (kraji[a][2]) - (kraji[d][2])
razdalja1 = math.sqrt((x**2) + (y**2))

x = kraji[a][1] - (kraji[c][1])
y = (kraji[a][2]) - (kraji[c][2])
razdalja2 = math.sqrt((x**2) + (y**2))

x = kraji[c][1] - (kraji[d][1])
y = (kraji[c][2]) - (kraji[d][2])
razdalja3 = math.sqrt((x**2) + (y**2))

s = (razdalja1 + razdalja2 + razdalja3) / 2
v = math.sqrt((s - razdalja1)*(s - razdalja2)*(s - razdalja3) * s)
print("Ploščina največjega trikotnika je", v)
